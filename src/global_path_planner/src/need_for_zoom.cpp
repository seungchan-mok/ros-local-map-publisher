...
int main(int argc, char **argv){
    ros::init(argc, argv, "need_for_zoom");
    ros::NodeHandle n;
    region_state_pub = n.advertise<std_msgs::String>("/zoom_state",1);
    ...
    cout << file_path;
    cv::Mat region_image = cv::imread(file_path+"/map_data/need_for_zoom.png",1);
    while (ros::ok())
    {

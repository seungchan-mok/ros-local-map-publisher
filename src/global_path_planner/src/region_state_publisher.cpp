...
const string state_table[11] = {"go","stop","cross_road","slow_down_for_traffic_light","static_obs","outbreak_obs","parking_search","parking","xxx","xxx","finish"};
...
void parking_numCallback(const std_msgs::Int16 msg)
{
  parking_num = msg.data;
}
void parking_stateCallback(const std_msgs::Int16 msg)
{
  parking_state = msg.data;
  if(parking_state == 3)
  {
      parking_done = true;
  }
...
int main(int argc, char **argv){
    ...
    ros::Subscriber parking_num_sub = n.subscribe("/parking_num", 1, parking_numCallback);
    ros::Subscriber parking_state_sub = n.subscribe("/parking_state", 1, parking_stateCallback);
...
//                cout << point << "\n";
                ...
                else
                {
                    ...
                    {
                        final_state = "go";
                        if(trf_point < 200 && traffic_light_state != "LEFT")
                        {
                            cout << "LEFT ONLY!\n";
                            final_state = "stop";
                        }
                    ...
                    if(state_string == "parking_search" && !parking_done && parking_num > 0)
                    {
                        final_state = "parking";
                    }
                    if(state_string == "parking_search" && parking_done)
                    {
                        final_state = "go";
                    }
                }

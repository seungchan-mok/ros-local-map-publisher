...
const string state_table[11] = {"go","stop","cross_road","slow_down_for_traffic_light","static_obs","outbreak_obs","parking_search","parking_in","parking_wait","parking_out","finish"};
...
int main(int argc, char **argv){
    ros::init(argc, argv, "traffic_state");
    ros::NodeHandle n;
    region_state_pub = n.advertise<std_msgs::String>("/traffic_region_state",1);
    ...
    cout << file_path;
    cv::Mat region_image = cv::imread(file_path+"/map_data/region_final.png",1);
    while (ros::ok())
    {
    ...
    //            cout << point << "\n";
                ...
                else
                {
                    ...
                    else if(state_string == "outbreak_obs"){
                         final_state = "outbreak_obs";
                    }
	            else
                    {
                         final_state = "go";
